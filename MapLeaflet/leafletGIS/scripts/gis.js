...
GIS.addGaotie = function(data, content) {
	...
	uniqueArr.map(function(lineName) {
		...
		//开始画线
		var opts = GIS.common.setOptions({weight: 4, color: '#76EE00', opacity: 0.8}, {name: lineName});
    ...
    var outServiceUrl = '/sml/query/highSpeed-cfg-queryTGCells';
    		  $.ajax({
    		        ...
    		        success : function(outServiceRes) {
    		        	  var data = outServiceRes.data;
    		        	  for (var i = 0; i < data.length; i++) {
    		        	  	    var currObj = data[i];
    		        	  	    outServiceStr += currObj.SECTION_NAME + ","
    		        	  }; 
    		        },
    		        beforeSend: function() {
    		        	$('body').mask('数据加载中，请稍后...');
    ...
    var warningUrl = '/sml/query/highSpeed-cfg-queryGCCells';
      $.ajax({
            ...
            success : function(warningRes) {
            	  var data = warningRes.data;
            	  for (var i = 0; i < data.length; i++) {
            	  	    var currObj = data[i];
            	  	    warningStr += currObj.SECTION_NAME + ","
            	  }; 
            },
            beforeSend: function() {
            	$('body').mask('数据加载中，请稍后...');
                                              ...
                                              success : function(warningRes) {
                                              	  ...
                                              	  /////////////////////////////
                                                      		...
                                                      		data.map(function(dataObj) {
                                                      			...
                                                      			if (outServiceStr.indexOf(section_name) > -1) {
                                                      				color = "#FF0000";
                                                      				isShowOutService = 1;
                                                      			};
                                                      			if (warningStr.indexOf(section_name) > -1) {
                                                      				color = "#FF0000";
                                                      				isShowWarning = 1;
                                                      			}
                                                                  ...
                                                                  //----------------------------------------------------------------------------------------------------- 
                                                                     ...
                                                                     //开始画线
                                                                     var opts_bac = GIS.common.setOptions({weight: 10, color: 'white', opacity: 0.8});
                                                                     var polyline_bac = L.polyline(points, opts_bac);
                                                                     //polyline_bac.addTo(GIS.layers[section_name]);

                                                                     var opts = GIS.common.setOptions({weight: 4, color: '#76EE00', opacity: 0.8}, {name: section_name, color: color});
                                                                     var polyline = L.polyline(points, opts);
...
///////////////////////////////////////////////////////////////////////////////////////////
function shousuoOrzhankai(obj){
       var $obj = $(obj);
       var src = $obj.attr('src'); 
       if (src.indexOf("shousuo") > -1) {
           $("#menueIndex").animate({width:'10px'},1000);
           $("#map").animate({left:'10px'},1000);
           $("#menutContent").css('display', 'none');
           $obj.attr('src','../../scripts/leafletGIS/images/zhankai.png');
           $("#table_jqGrid").animate({left:'10px'},1000,function(){
           	         $("#con_grid_div_grid_road").setGridWidth($("#table_jqGrid").width());
           });
       }else{
           $("#menueIndex").animate({width:'235px'},1000);
           $("#map").animate({left:'246px'},1000);
           $("#menutContent").css('display', 'block');
           $obj.attr('src','../../scripts/leafletGIS/images/shousuo.png');
           $("#table_jqGrid").animate({left:'246px'},1000,function(){
           	         $("#con_grid_div_grid_road").setGridWidth($("#table_jqGrid").width()+14);
           });
       }
...
function initJqgrid(){
	    var colNames=['时间','高铁线路', '路段', '方向', 'VOLTE语音接通率（%）','VOLTE语音掉话率（%）','ESRVCC切换成功率（%）','CSFB被叫接通率（%）','CSFB掉话率（%）'];
	    var colModel=[
	                   {name:'id',index:'id', width:260,align:"center"},
	                   {name:'userName',index:'userName', width:390,align:"center"},
	                   {name:'gender',index:'gender', width:490,align:"center"},
	                   {name:'email',index:'email', width:325,sorttype:"string",align:"center"},
	                   {name:'QQ',index:'QQ',width:100,align:"center"},                
	                   {name:'mobilePhone',index:'mobilePhone', width:220,align:"center"},                
	                   {name:'mobilePhone',index:'mobilePhone', width:220,align:"center"},                
	                   {name:'mobilePhone',index:'mobilePhone', width:220,align:"center"},                
	                   {name:'birthday',index:'birthday', width:238, sorttype:"date",align:"center"}               
	               ];
